...
//    데이터베이스에 SQL 문을 전다랗여 실행하기 위한 클래스 (단순 쿼리문용)
...
//            xml 파일에서 특수기호가 사용 시 정상적으로 동작하지 않음
//            데이터베이스 접속 url의 옵셕 부분을 따로 분리하여 사용
//            xml 파일에서 html특수기호를 사용 시 옵션을 분리하지 않고 한번에 입력할 수 있음
            dbUrl += "?" + dbOpt1 + "&" + dbOpt2;
