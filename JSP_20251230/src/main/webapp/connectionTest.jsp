...
//  데이터베이스 접속은 외부 리소스를 사용하는 것이므로 반드시 try ~ catch로 예외처리를 해야 함
  try {
//      데이터베이스 서버 접속 주소 설정
...
//      port : 네트워크 사용 시 IP번호가 1개 주어지기 때문에 효과적으로 사용할 수 없음, 네트워크 자원을 port라는 것으로 나누워서 동시에 여러가지 네트워크를 사용할 수 있도록 함
//      1 ~ 65535 번까지 존재함, 1 ~ 1000번까지는 OS 내부적으로 사용한 것이 많기 때문에 1000번 이후 번호를 사용하는 것이 좋음
...
//    getConnection() : 지정한 URL과 사용자 ID/PW를 사용하여 실제로 데이터베이스 접속
...
//  데이터베이스는 외부 리소스이기 때문에 finally를 사용하여 반드시 리소스를 해제해야 함
  finally {
//      Connection 클래스 타입의 변수 conn이 null이 아닐 경우는 한번 이상 데이터베이스에 접속했다는 의미
//      conn이 null일 경우는 데이터베이스에 접속하지 않았다는 의미
    if (conn != null) {
//        데이터베이스 접속 끊기, 리소스 해제
      conn.close();
    }
